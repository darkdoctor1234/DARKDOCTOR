...
const TAB_META: Record<Tab, { label: string; empty: string; emptyDesc: string }> = {
  ...
  history:  { label: "History",  empty: "Nothing yet",   emptyDesc: "Resolved reviews will show up here." },
};
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [p, r, h] = await Promise.all([
        ...
        collegeApi.reviews.admin.history(),
      ]);
      setPending(p); setReported(r); setHistory(h);
    } catch { setPending([]); setReported([]); setHistory([]); }
      ...
      <main style={{ maxWidth: "860px", margin: "0 auto", padding: "40px 20px 80px" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <p style={{ fontSize: "0.875rem", color: "var(--dd-text3)" }}>
            Every new review needs approval before it's published. Reviews already live get a second look once reported 5+ times.
            This is the same queue <code style={{ fontSize: "0.8em" }}>admin</code> accounts work from. You can see and act on everything they do, plus a history of who resolved what.
          </p>
        </div>

        {/* Tabs */}
        <div style={{ display: "flex", gap: "8px", marginBottom: "24px", flexWrap: "wrap" }}>
          {(["pending", "reported", "history"] as Tab[]).map((t) => {
            const count = t === "pending" ? pending.length : t === "reported" ? reported.length : history.length;
            const active = tab === t;
            return (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {activeList.map((review) => (
              <AdminReviewCard
                key={review.id}
                review={review}
                mode={tab === "pending" ? "pending" : tab === "reported" ? "reported" : "history"}
